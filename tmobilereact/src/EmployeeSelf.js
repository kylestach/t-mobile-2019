...
import {activateEmployee, deactivateEmployee, getEmployees} from "./schedules";
...
class EmployeeSelf extends React.Component {
    ...
    get currentName() {
        return this.state.employee ? this.state.employee.name : 'Failed to load user';
    }
    ...
    onToggleActive = () => {
        ...
        if (this.isActive) {
            ...
            activateEmployee(this.state.employee).then(() => {
                const nUser = clone(this.state.employee);
                nUser.active = true;
                this.setState({employee: nUser});
            }).catch(console.error);
